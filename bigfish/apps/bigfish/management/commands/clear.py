...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--delete-file',
                            ...
                            dest='delete-file',
                            default=False,
                            help='only delete the migrations files')
    ...
    def delete_files_app(self, app):
        ...
        if os.path.exists(migrations_dir):
            shutil.rmtree(os.path.join(BASE_DIR, 'apps', app, 'migrations'))
    ...
    def handle(self, *args, **options):
        ...
        if apps == ['all']:
            for item in Public._meta.apps.all_models.items():
                self.delete_files_app(item[0])
        elif apps == ['self']:
            for item in settings.INSTALLED_APPS:
                if not item.startswith("bigfish.apps."):
                    continue
                self.delete_files_app(item.replace("bigfish.apps.", ""))
        else:
